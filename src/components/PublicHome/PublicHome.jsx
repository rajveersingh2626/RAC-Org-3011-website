...
// Accordion Slices Data for Flagship Projects
...
// Matrix Text Scramble Title Component (Fixed selection & layout shift)
...
// Smooth Typewriter Text Component
...
// Big Rotary Wheel Component with Continuous Ambient Slow Rotation & Ultra-Smooth Lerped Motion
function BigRotaryWheel({ containerRef }) {
  ...
  useEffect(() => {
    ...
    const updateFrame = () => {
      // 1. Continuous slow ambient rotation (~3 degrees per second)
      ambientRotationRef.current += 0.06;

      // 2. Calculate targets based on scroll position
      const scrollY = scrollYRef.current;
      const scrollProgress = Math.min(1, Math.max(0, scrollY / 450));

      const targetScale = 1.0 - scrollProgress * 0.28; // Smoothly scale down to 0.72
      const targetLeft = 92 + scrollProgress * 6;      // Smoothly shift to 98%

      // 3. Silky smooth exponential lerp (0.08 factor) in both scroll directions (no wobble/twisting)
      ...
      // 4. Combined rotation: continuous slow ambient spin + scroll spin
      const totalRotation = ambientRotationRef.current + scrollY * 0.12;

      // 5. Apply direct DOM updates for 60fps performance without layout thrashing
      if (wheelRef.current) {
...
// Flagship Projects Data for Interactive K-Wheel
const BASE_PROJECTS = [
...
export default function PublicHome({ onNavigateDistrict, onOpenLoginModal }) {
  ...
  useEffect(() => {
    ...
    // Check if hover is supported (Desktop experience)
    ...
    const handleWheel = (e) => {
      ...
      // Reset inertia debounce on every trackpad momentum pulse so momentum doesn't skip a 2nd section
  ...
  // Active Accordion Slice State
  const [activeSlice, setActiveSlice] = useState(1);

  // Join Form Modal State
  ...
  // Impact Calculations based on contribution amount
  ...
  return (
    <div ref={containerRef} className="snap-container" style={{ backgroundColor: '#FFFFFF' }}>
      ...
      {/* Big Rotary Wheel Background Motion */}
      <BigRotaryWheel containerRef={containerRef} />

      {/* SECTION 1: HERO SECTION (100VH SCREEN WIDE) */}
      <section
        ...
        >
          <h1
            style={{
              ...
              fontWeight: 900,
              color: '#1a1a1a', // Near black as in image
              lineHeight: 0.95,
              margin: '0 0 16px 0',
              letterSpacing: '-1.5px',
              textTransform: 'uppercase',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px', flexWrap: 'wrap' }}>
            <span
              style={{
                ...
                fontWeight: 900,
                color: '#0044ff', // Bright blue as in image
                lineHeight: 0.85,
                letterSpacing: '-3px'
              }}
            >
      ...
      {/* SECTION 2: ROTATING K-WHEEL FLAGSHIP PROJECTS SECTION (LIGHT THEME) */}
      ...
      {/* SECTION 3: DYNAMIC IMPACT CALCULATOR BAR (100VH SCREEN WIDE) */}
      <section className="snap-section" style={{ background: 'linear-gradient(180deg, #FFFFFF 0%, #FDF8FA 100%)', padding: '40px 32px' }}>
        <div className="section-content-animate" style={{ maxWidth: '1320px', width: '100%', position: 'relative', zIndex: 10 }}>
          ...
          {/* Interactive Controls Area (Full Section Wide) */}
          <div
            style={{
              background: '#FFFFFF',
              padding: '28px 36px',
          ...
          >
            {/* Top row: Label & Amount Input Display Box */}
          ...
          {/* Calculated Impact Grid Across Full Width */}
      ...
      {/* SECTION 4: IMPACT METRICS SECTION (100VH SCREEN WIDE) */}
      ...
      {/* SECTION 5: ROTARY & ROTARACT FOCUS AREAS (FIT IN VIEWPORT WITHOUT CUTOFF) */}
      ...
      {/* JOIN INTEREST POPUP MODAL (USER REQUESTED FORM) */}
      {isJoinModalOpen && (
        <div
          style={{
            position: 'fixed',
