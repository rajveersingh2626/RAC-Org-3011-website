...
export default function LoginModal({ onClose, onLoginSuccess }) {
  ...
  // Authenticated user details & TOTP secret
  const [authenticatedUser, setAuthenticatedUser] = useState(null);
  const [totpSecret, setTotpSecret] = useState('');
  const [showKeyDetails, setShowKeyDetails] = useState(false);
  const [copiedKey, setCopiedKey] = useState(false);
  
  // 2FA OTP Code (Google Authenticator)
  const [otpCode, setOtpCode] = useState('');
  
  // Forgot Password Rate Limiting & State
  ...
  // Step 2: Submit 6-digit Google 2FA Code
  ...
  // Forgot Password Submit with Capping / Rate Limiting
      ...
      >
        ...
        {!isVerifying && (
          <>
            ...
            {/* MODE 1: ROTARY ID & PASSWORD LOGIN */}
            {mode === 'login' && (
              <form onSubmit={handleCredentialsSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <input
                      type="text"
                      placeholder="e.g. 10482950 or 3011-PRES-01"
                      required
                      value={rotaryId}
            ...
            {/* MODE 2: GOOGLE 2FA AUTHENTICATOR CODE */}
            {mode === 'google2fa' && (
              <form onSubmit={handleOtpSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                {/* Secret Key Setup Accordion for Google Authenticator App */}
